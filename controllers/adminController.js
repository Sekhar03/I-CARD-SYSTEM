...
router.get('/applications/:id', async (req, res) => {
  try {
    ...
    if (mongoose.Types.ObjectId.isValid(id)) {
      application = await Gazetted.findById(id);
      type = 'gazetted';
      if (!application) {
    ...
    // If not found, try by applicationNo
    if (!application) {
      application = await Gazetted.findOne({ applicationNo: new RegExp('^' + id + '$', 'i') });
      type = 'gazetted';
      if (!application) {
        application = await NonGazetted.findOne({ applicationNo: new RegExp('^' + id + '$', 'i') });
        type = 'non-gazetted';
      }
    }
    if (!application) {
...
router.post('/report', async (req, res) => {
  try {
    const { startDate, endDate, reportType, quickType, format } = req.body;
    const filters = {};
    // Date range filter
    ...
    if (reportType && reportType !== 'all') {
      if (reportType === 'gazetted') filters.type = 'gazetted';
      else if (reportType === 'non-gazetted') filters.type = 'non-gazetted';
      else if (reportType === 'approved') filters.status = 'Approved';
      else if (reportType === 'rejected') filters.status = 'Rejected';
    }
    // Quick report filter
    if (quickType) {
      const now = new Date();
      if (quickType === 'today') {
        ...
        filters.createdAt = { $gte: start };
      } else if (quickType === 'pending') {
        filters.status = 'Pending';
      } else if (quickType === 'printed') {
        filters.status = 'Printed';
      } else if (quickType === 'approved') {
        filters.status = 'Approved';
      } else if (quickType === 'rejected') {
        filters.status = 'Rejected';
      }
    }
    // Fetch data from both collections
    const gazetted = (filters.type && filters.type !== 'gazetted') ? [] : await Gazetted.find(filters);
    const nonGazetted = (filters.type && filters.type !== 'non-gazetted') ? [] : await NonGazetted.find(filters);
    const data = [
      ...gazetted.map(app => ({ ...app.toObject(), type: 'gazetted' })),
      ...nonGazetted.map(app => ({ ...app.toObject(), type: 'non-gazetted' }))
    ];
    if (format === 'excel') {
      const workbook = new ExcelJS.Workbook();
      const worksheet = workbook.addWorksheet('Report');
      worksheet.columns = Object.keys(data[0] || {empty: ''}).map(key => ({ header: key, key }));
