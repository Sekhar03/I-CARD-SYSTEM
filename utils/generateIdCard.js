...
async function generateIdCard(application, type) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // Generate random Sl. No.
      let slNoPrefix = type === 'gazetted' ? 'GZ' : 'NG';
      let slNoNumber = String(Math.floor(Math.random() * 90) + 10); // random 2-digit number
      let slNo = `${slNoPrefix}/${slNoNumber}`;
      ...
      const headerBottomY = textBlockStartY + hindiFontSize + engFontSize + 10;
      // Set stripesTopY just below the header
      const stripesTopY = headerBottomY + 6;
      ...
      // Move Hindi text to align with the English serial number
      renderHindiText('प्र.का', 150, stripesTopY + 23, { fontSize: 8 });
      doc.font('Helvetica-Bold')
         .fontSize(8)
         .fillColor('#ffffff')
         .text(`H.Q. SI.No. COMMERCIAL- ${slNo}`, 150, stripesTopY + 32);
